...
<div class="grid grid-cols-6 gap-2 mt-4">
   ...
   <div class="max-w-sm rounded overflow-hidden shadow-lg grid col-span-6 sm:col-span-2">
    <div class="px-6 py-4">
        <div class="">
            <div class="font-bold text-xl text-indigo-700 mb-2 hover:text-indigo-500"> <a href="/admin/tasks/{{$task->id}}">{{$task->title}}</a> </div>
            <p class="text-gray-700 text-base mb-6">
              {{Str::limit($task->description, 80, '...')}}
            </p>
        </div>
      <div class="flex justify-between">
          <a class="border-2 font-bold border-indigo-700 px-5 py-2 rounded-lg text-indigo-700 hover:bg-indigo-700 hover:text-indigo-50 hover:shadow-md transition duration-300 ease-in-out" href="/admin/tasks/{{$task->id}}">
            Read More
          </a>
